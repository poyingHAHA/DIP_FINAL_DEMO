...
def create_filters():
    sobel_x = tf.constant([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]], dtype=tf.float64)
    sobel_y = tf.transpose(sobel_x)

    sobel_x_filter = tf.stack([sobel_x] * 3, axis=2)
    sobel_x_filter = tf.stack([sobel_x_filter] * 3, axis=3)

    sobel_y_filter = tf.stack([sobel_y] * 3, axis=2)
    sobel_y_filter = tf.stack([sobel_y_filter] * 3, axis=3)
